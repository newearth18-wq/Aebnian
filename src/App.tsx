...
export default function App() {
  ...
  async function openRoom() {
    ...
    const packId = packs.find((pack) => pack.id === selectedPack)?.id || packs[0]?.id || "";
    if (!session || !packId) { setMessage("เข้าสู่ระบบและเลือกชุดคำถามก่อน"); return; }
    try {
      const joined = await gameClient.create("classroom", { packId, mapId, durationMinutes: duration, teacherToken: session.access_token, nickname: "ผู้สอน" });
